# inspired by https://adventuresinmachinelearning.com/keras-lstm-tutorial/
...
data_path = "popular_quotes_clean"
...
def file_to_word_ids(df, word_to_id):
...
def load_data():
    ...
    left = df.drop(train.index)
    valid = left.sample(frac=0.5, random_state=420)
    test = left.drop(valid.index)
    # build the complete vocabulary, then convert text data to list of integers
    word_to_id = build_vocab(df)
    train_data = file_to_word_ids(train, word_to_id)
    valid_data = file_to_word_ids(valid, word_to_id)
    test_data = file_to_word_ids(test, word_to_id)
